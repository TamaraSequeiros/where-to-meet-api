...
const calculate_middle = async (location1, location2) => {
    ...
    if (response.hasError) {
        ...
        throw new Error('Error retrieving routes');
    }
    return process_route(response.data);
};

const process_route = (route_data) => {
    const route = route_data.routes && route_data.routes[0];
    if (!route) {
        throw new Error('No bicycle route found between these locations');
    }
    ...
    if (!location) {
        throw new Error('Could not determine a halfway point along the route');
    }
